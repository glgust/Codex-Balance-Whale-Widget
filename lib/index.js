...
const DSH_HOME = process.env.WHALE_HOME || process.env.DSH_HOME || path.join(os.homedir(), '.dsh')
  ...
  apply(ctx) {
    ...
    function parseCodexFile(file) {
      ...
      for (const line of text.split('\n')) {
        ...
        if (!ts) continue
        // 额度快照独立于 token 增量：重复计数/仅额度事件也可以刷新窗口。
        const rl = p.rate_limits
        if (rl && typeof rl === 'object' && (rl.primary || rl.secondary || rl.plan_type || rl.planType || rl.credits)) {
        ...
        // 该次增量
        let delta = null
        const rawTotal = tot && tot.total_tokens
        const totAll = rawTotal === null || rawTotal === undefined || rawTotal === '' ? NaN : Number(rawTotal)
        if (Number.isFinite(totAll) && totAll >= 0) {
    ...
    function normalizeCodexWindow(w) {
      ...
      if (secs !== null && secs >= 0) resetAt = Date.now() + secs * 1000
        ...
        const abs = w.resets_at || w.resetsAt || w.reset_at || w.reset_time || w.next_reset || w.resetAt || w.nextResetTime
      ...
      const windowMinutes = pick(['window_duration_mins', 'windowDurationMins', 'window_minutes', 'windowMinutes', 'window', 'period_minutes'])
    ...
    function normalizeCodexRateLimits(rl) {
      ...
      return {
        primary, secondary,
        planType: String(rl.plan_type || rl.planType || ''),
    ...
    if (!ctx.standalone) try {
      const warmup = setTimeout(function () { try { codexSummaryCached() } catch (err) {} }, 1500)
      const polling = setInterval(function () { try { codexSummaryCached() } catch (err) {} }, 5 * 60 * 1000)
      disposers.push(() => { clearTimeout(warmup); clearInterval(polling) })
    } catch (err) {}
    // ===== 自定义 API 模型注册表（v655）=====
